...
SELECT p.process_id, l.cwDocId, l.serviceNumber, l.cwOrderId, l.workOrderNumber, l.elementTypeInOrderTree, h.orderNumber, w.element_type, w.element_inst_id, v.status_name
  FROM cwprocess_not_completed p, cwmdtypes t, stc_lineitem l, stc_bundleorder_header h, work_order_inst@rms_prod_db_link w, val_task_status@rms_prod_db_link v
 WHERE p.process_metadatype = t.typeid
   AND t.typename in ('processSTC.mainSTCProvisiongProcess', 'processSTC.mainSTCProvisiongProcess_NoParticipant', 
                      'processSTC.mainSTCProvisiongProcess_NoParticipant_2', 'processSTC.mainSTCProvisiongProcess_NoParticipant_3',
                      'processSTC.mainSTCProvisiongProcess_NoParticipant_4')
   AND p.status = 1
