CREATE OR REPLACE PROCEDURE CreateLegacyOrderFromGranite (newOrderNumber IN VARCHAR2, errorMsg OUT VARCHAR2) AS
...
BEGIN
  ...
  BEGIN
    SELECT wo_inst_id, circ_path_inst_id, circ_path_hum_id
      INTO wOrdInstId, pathInstId, pathHumId
      FROM circ_path_inst@rms_prod_db_link, work_order_inst@rms_prod_db_link
     WHERE circ_path_inst_id = element_inst_id
       AND wo_name = newOrderNumber
       AND element_type = 'P';
  EXCEPTION
    WHEN no_data_found THEN
      errorMsg := '[ERR] Unable to find WorkOrder and/or Path; one or both are missing in online tables!';
      RETURN;
  END;
  ...
  IF(stopRun = 'Y') THEN
    errorMsg := '[ERR] Order already exists in WD: orderStatus = '||existOrderStatus||'; serviceNumber = '||existServiceNumber||
...
END CreateLegacyOrderFromGranite;
/