...
dbms_output.enable(100000000);
  ...
  INSERT INTO rmsh_cw.cwprocesseventlog(event_id,                    transaction_id, event_source, event_severity,    event_code,
                                        description,                                                
                                        event_time, user_id,                       stack_trace, node_id)
                                 values(rmsh_cw.cwfEventSeq.nextval, 0,              1,            3,              9995,
                                        'Running with ('||submitted_date_year||','||order_type||');', 
                                        sysdate,    'FILL_HBU_EXPD_ORDER_MISSING', null,        'HBU');
  
      
  EXECUTE IMMEDIATE('TRUNCATE TABLE expd_hbu_order_missing');
  
  EXECUTE IMMEDIATE('TRUNCATE TABLE tmptable_granite_data_for_hbu');
      ...
      WHEN others THEN
        ...
        INSERT INTO rmsh_cw.cwprocesseventlog(event_id,                    transaction_id, event_source, event_severity,    event_code, 
                                       ...
                                       values(rmsh_cw.cwfEventSeq.nextval, 0,              1,            3,              9995,       
                                              ...
                                              'Error while loading data for order '||i.wo_name||' - step '||step||': '||error_msg,
                                              'HBU');
