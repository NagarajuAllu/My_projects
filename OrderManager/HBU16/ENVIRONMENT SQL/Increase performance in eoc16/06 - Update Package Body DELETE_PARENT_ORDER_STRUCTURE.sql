CREATE OR REPLACE PACKAGE BODY DELETE_PARENT_ORDER_STRUCTURE IS
  ...
  BEGIN
    ...
    SELECT COUNT(1)
      ...
      FROM work_order_inst@rms_prod_db_link woi, workorder_attr_settings@rms_prod_db_link was, val_attr_name@rms_prod_db_link van
  ...
  FUNCTION validateCancelChildOrder(order_number IN VARCHAR2) return CHAR IS
    ...
    count_processes NUMBER(5);
    work_order_status  work_order_inst.status@rms_prod_db_link%TYPE;
    status_name        val_task_status.status_name@rms_prod_db_link%TYPE;


  BEGIN
    BEGIN
      SELECT status, (SELECT status_name FROM val_task_status@rms_prod_db_link WHERE stat_code = status) status_name
    ...
    BEGIN
      SELECT count(*) 
        ...
        FROM cwprocess_not_completed
       WHERE order_id = (SELECT cworderid 
                           FROM stc_order_message_home
      ...
      IF (count_processes = 0) THEN
        result := 'Y';
      ELSE 
        RAISE_APPLICATION_ERROR(-20106, 'ORDER: '||order_number||' - FOUND '||count_processes||' PROCESSES RUNNING IN EOC DB FOR THIS ORDER');
      END IF;
