CREATE OR REPLACE PACKAGE BODY DELETE_PARENT_ORDER_STRUCTURE IS
  ...
  BEGIN
    ...
    SELECT COUNT(1)
      ...
      FROM rms_prod.work_order_inst@rms_prod_db_link woi, rms_prod.workorder_attr_settings@rms_prod_db_link was, rms_prod.val_attr_name@rms_prod_db_link van
  ...
  FUNCTION validateCancelChildOrder(order_number IN VARCHAR2) return CHAR IS
    ...
    work_order_status  rms_prod.work_order_inst.status@rms_prod_db_link%TYPE;
    status_name        rms_prod.val_task_status.status_name@rms_prod_db_link%TYPE;


  BEGIN
    BEGIN
      SELECT status, (SELECT status_name FROM rms_prod.val_task_status@rms_prod_db_link WHERE stat_code = status) status_name
