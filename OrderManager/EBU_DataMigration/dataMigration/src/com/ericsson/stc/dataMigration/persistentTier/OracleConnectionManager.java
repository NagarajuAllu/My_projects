...
public class OracleConnectionManager {
  ...
  /**
   * To update the record in STC_CUSTOMER_MIGRATION identified by id with the provided result.
   */
  ...
  /**
   * To update the record in STC_SERVICE_MIGRATION identified by orderNumber with the provided result.
   */
  public void markServiceRecordAsProcessed(String pathName, Integer resultValue, String errorResult, String orderNumber) {
    Connection connection = null;
    ...
    try {
      ...
      if (connection != null) {
        if(_updateStatementForService != null) {
          ...
          preparedStatementForUpdate.setInt(1, resultValue);
          preparedStatementForUpdate.setString(2, (errorResult != null ?
                                                   errorResult.substring(0, (errorResult.length() < 1000 ? errorResult.length() : 1000)) :
                                                   null));
          preparedStatementForUpdate.setString(3, orderNumber);
          int rowsUpdated = preparedStatementForUpdate.executeUpdate();
          if(rowsUpdated > 0) {
            _logger.debug("Record in SERVICE with <" + pathName + "," + orderNumber + "> updated with value '" + resultValue + "'");
          }
          else {
            _logger.error("Unable to find record in SERVICE with <" + pathName + "," + orderNumber + ">");
          }
          preparedStatementForUpdate.close();
